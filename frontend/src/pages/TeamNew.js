...
const TeamNew = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await profileService.createTeam(submitData); // Use profileService
      navigate(`/teams/${response.data.id}`); // Navigate to the new team detail page
    } catch (err) {
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="flex justify-end">
              ...
              <button
                ...
                className={`flex items-center bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline ${isSubmitting || !formData.organization_id ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
